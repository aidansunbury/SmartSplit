...
export function Members() {
  ...
  return (
    <div className="flex flex-col space-y-4">
      <h1 className="text-l font-bold">Group Members</h1>
      {members.users.map((user) => {
        const isNegative = user.balance < 0;

        return (
          <div className="flex flex-row items-center space-x-4" key={user.user.id}>
            <Avatar>
              <AvatarImage src={user.user.image} />
            </Avatar>
            <div className="flex flex-col">
              <h2 className="font-medium">{user.user.name}</h2>
              <div className={`${isNegative ? "text-orange-500" : "text-green-500"} text-sm`}>
